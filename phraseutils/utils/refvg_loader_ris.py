...
class RefVGLoader(object):
    ...
    def __init__(self, split=None, phrase_handler=None, word_embed=None, allow_no_att=True, allow_no_rel=True,
                 include_vg_scene_graph=False, input_anno_only=False, save_json=False):
        ...
        for task in ref_tasks:
            ...
            del task['phrase_structure'], task['instance_boxes'], task['ann_ids'], task['image_id'], task['task_id']
            self.TasksRefer[task_id] = task
        ...
        print("The size of the dictionary is {} bytes".format(sys.getsizeof(self.ImgReferTasks)))
        print("The size of the dictionary is {} bytes".format(sys.getsizeof(self.TasksRefer)))
        # import ipdb; ipdb.set_trace()
        self.img_ids = list(self.ImgReferTasks.keys())
        self.task_ids = list(self.TasksRefer.keys())
        # self.shuffle()
        self.iterator = 0
        ...
        # import ipdb; ipdb.set_trace()
        if save_json:
            with open("%s/refer_%s_ris.json" % (os.path.dirname(refer_fpaths[self.splits[0]]), self.splits[0]), "w") as json_file:
                json.dump(self.TasksRefer, json_file)
